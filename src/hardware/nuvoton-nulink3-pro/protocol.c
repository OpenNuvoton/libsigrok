...
#define MAX_LOGIC_SAMPLERATE    SR_MHZ(55)
...
#define TRANSFER_BUFFER_SIZE    (USB_PACKET_SIZE * 128)
...
static void deinterleave_buffer(const uint8_t *src, uint32_t length, uint8_t *dst_ptr, uint32_t channel_count, uint32_t channel_mask)
{
    ...
    for (const uint32_t *src_ptr = (const uint32_t *)src; src_ptr < ((const uint32_t *)src + length / 4); src_ptr += channel_count)
    {
        for (int32_t bit = 0; bit != 32; bit++)
        {
            const uint32_t *channel_ptr = (uint32_t *)src_ptr;
...
static int32_t start_transfers(const struct sr_dev_inst *sdi, uint32_t channel_mask, uint32_t transfer_size)
{
    ...
    if (devc->cur_samplerate >= SR_MHZ(55))
        num_transfers *= 2;
...
SR_PRIV int32_t nubridge_acquisition_start(const struct sr_dev_inst *sdi)
{
    ...
    usb_source_add(sdi->session, devc->ctx, 5, receive_data, drvc);
